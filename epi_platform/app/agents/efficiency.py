...
class EfficiencyAdvisorAgent:
    ...
    def process_message(cls, user_message: str, chat_history: Optional[List[dict]] = None) -> dict:
        ...
        if model.startswith("gpt-5"):
            # Este agente razona sobre una instalacion existente en texto
            # libre: usamos "medium" (mas criterio) en vez del "low" de la
            # entrevista, que solo necesita extraer datos rapido.
            kwargs["reasoning_effort"] = os.getenv("EPI_LLM_REASONING_EFFORT_EFFICIENCY", "medium")
        else:
